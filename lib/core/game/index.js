...
var Game = exports.Game = function (_Emitter) {
  ...
  _createClass(Game, [{
    ...
    key: 'settle',
    value: function settle(participant, junc) {
      var collect = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : true;


      if (junc.owner === participant) throw new _utils.EdgeExecutionError('You have already settled here');

      if (!!junc.owner) throw new _utils.EdgeExecutionError('Someone has already settled here');

      if (!junc.isSettleable) throw new _utils.EdgeExecutionError('You can\'t settle next an existing settlement');
      ...
      junc.getNeighbors().forEach(function (neighbor) {
    ...
    value: function updateLongestRoad() {
      console.log(new _utils.CatonlineError('not implemented')); // $TODO
    }
  }, {
    key: 'equals',
