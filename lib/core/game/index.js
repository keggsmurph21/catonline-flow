  ...
  _createClass(Game, [{
    ...
    value: function mutate(participant, name, rawArgs) {
      ...
      try {
        ...
        if (e instanceof _utils.CatonlineError) return null;
    ...
    value: function equals(game) {
      ...
      return true && function () {
        ...
        if (equal) _underscore2.default.each(thisConds.board.hexes, function (hex, i) {
          equal = equal && (0, _utils.objectsMatch)(hex, thatConds.board.hexes[i]);
        });

        if (equal) _underscore2.default.each(thatConds.board.hexes, function (hex, i) {
          equal = equal && (0, _utils.objectsMatch)(hex, thisConds.board.hexes[i]);
        });

        if (equal) _underscore2.default.each(thisConds.board.ports, function (portType, i) {
          equal = equal && portType === thatConds.board.ports[i];
        });

        if (equal) _underscore2.default.each(thatConds.board.ports, function (portType, i) {
        ...
        return memo && item.edge.name === other.edge.name && item.args.toString() === other.args.toString() && item.participant.num === other.participant.num && item.result === other.result;
      }, true);
    }
  }, {
    key: 'serialize',
    value: function serialize() {
      ...
      return {
        ...
        playerIDs: this.participants.map(function (participant) {
    ...
    value: function deserialize(serial) {
      ...
      var originalOwner = new _player.Human(serial.ownerID);
      var originalPlayers = {};
      serial.playerIDs.forEach(function (id) {
      ...
      serial.history.forEach(function (item) {
        ...
        var participant = game.participants[item.participantNum];
        if (item.result === null) {
